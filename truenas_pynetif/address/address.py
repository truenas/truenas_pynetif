...
def replace_address(
    ...
    *,
    ...
    """Replace an IP address on an interface, resetting it to permanent.

    This is used to re-apply a static address configuration, clearing any
    valid_lft (valid lifetime) and preferred_lft (preferred lifetime) values
    that may have been set by DHCP or IPv6 SLAAC. After replacement, the
    address will have infinite lifetime (permanent).
    ...
    # Send message with REPLACE flag
    msg = pack_nlmsg(
        RTMType.NEWADDR,
        NLMsgFlags.REQUEST | NLMsgFlags.ACK | NLMsgFlags.REPLACE,
        ifaddrmsg + attrs,
    )
    sock.send(msg)
    recv_msgs(sock)
